  ...
  If lvwPrinter.ListItems.Count > 0 Then    ' Make sure we have printers
    ...
    If lvwPrinter.SelectedItem.Index > 0 And lvwPrinter.SelectedItem.Checked = False Then
      ...
      If chkConfirm.Value = vbChecked Then
        If MsgBox("Impresora: " & vbCrLf & sNewDefault & vbCrLf & _
                  "Selecionada Predeterminada" & vbCrLf & vbCrLf & "Esta seguro?", _
                  vbYesNo + vbDefaultButton2, "Impresora Predeterminada") = vbNo Then
          Exit Sub
        End If
      End If
...
'    lvwPrinter.ColumnHeaderIcons = imlLV2       ' For WinZip type sorting arrow use imlLV2
  ...
  If GetString(HKEY_CURRENT_USER, APPREGPATH, "Shutdown Properly") = "0" And _
    ...
    If MsgBox("Esta Impresora esta configurada ." & vbCrLf & _
              sOrigPrinter & vbCrLf & "como predeterminada" & vbCrLf & vbCrLf & _
              "Usted quiere restaurarla ahora?", vbQuestion + vbYesNo + vbDefaultButton2, "Restaurar Predeterminada") = vbYes Then
